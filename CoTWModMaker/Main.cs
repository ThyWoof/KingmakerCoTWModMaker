    ...
    internal class Main
    {
        internal class Settings
        {
            internal Settings()
            {
                ...
                using (StreamReader settings_file = File.OpenText("Mods/CoTWModMaker/settings.json"))
                using (JsonTextReader reader = new JsonTextReader(settings_file))
                {
                    JObject jo = (JObject)JToken.ReadFrom(reader);

                    //
                    // Assign your MOD Settings here to local properties
                    //
                }
            }
        }
        ...
        static class LibraryScriptableObject_LoadDictionary_Patch
        {
            static void Postfix(LibraryScriptableObject __instance)
            {
                ...
                try
                {
                    Main.DebugLog("Loading CoTW Mod Maker");

                    CallOfTheWild.LoadIcons.Image2Sprite.icons_folder = @"./Mods/CoTWModMaker/Icons/";
#if DEBUG                
...
#if DEBUG
                    string guid_file_name = @"./Mods/CoTWModMaker/blueprints.txt";
                    CallOfTheWild.Helpers.GuidStorage.dump(guid_file_name);
#endif
                    CallOfTheWild.Helpers.GuidStorage.dump(@"./Mods/CoTWModMaker/loaded_blueprints.txt");
                }
                catch (Exception ex)
                {
                    Main.DebugError(ex);
